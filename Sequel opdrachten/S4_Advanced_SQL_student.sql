...
DROP VIEW IF EXISTS s4_1; CREATE OR REPLACE VIEW s4_1 AS                                                     -- [TEST]
	select m.mnr, m.functie, m.gbdatum
	from medewerkers m
	where (m.functie = 'VERKOPER' 
	or m.functie = 'TRAINER')
	and m.gbdatum < '1980-01-01'::date
	order by m.gbdatum;
...
DROP VIEW IF EXISTS s4_3; CREATE OR REPLACE VIEW s4_3 AS                                                     -- [TEST]
	select i.cursus, i.begindatum, count(*)
...
-- 	SELECT avg(round(months_between(now(), m.gbdatum) / 12)) leeftijd, m.naam
-- 	from medewerkers m
-- 	group by m.naam;
...
DROP VIEW IF EXISTS s4_7; CREATE OR REPLACE VIEW s4_7 AS                                                     -- [TEST]
	select count(*) as aantal_medewerkers, sum(comm)/count(*) as commissie_verkopers, avg(comm) as commissie_medewerkers
	from medewerkers;
