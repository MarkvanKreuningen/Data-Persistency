...
insert into medewerkers values (7001, 'KREUNINGEN', 'M', 'VERKOPER', 7902, '12-09-1998', 2800.00, 500.00, 30, 'm')
...
insert into medewerkers values (8000, 'DONK', 'A', 'MANAGER', 7839, '12-09-1980', 3500.00, NULL, 50, 'm')
ON CONFLICT DO NOTHING;
...
DROP SEQUENCE IF EXISTS afd_nr_seq;
CREATE SEQUENCE afd_nr_seq
    MINVALUE 10
    MAXVALUE 100
    START WITH 60
    INCREMENT BY 10;

insert into afdelingen values (nextval('afd_nr_seq'), 'DIAGRAMMEN', 'MIJDRECHT', NULL)
ON CONFLICT DO NOTHING;
insert into afdelingen values (nextval('afd_nr_seq'), 'TESTEN', 'MIJDRECHT', NULL)
ON CONFLICT DO NOTHING;
insert into afdelingen values (nextval('afd_nr_seq'), 'SOFTWARE', 'MIJDRECHT', NULL)
ON CONFLICT DO NOTHING;
...
CREATE TABLE adressen (
                          ...
                          CONSTRAINT med_mnr_fk FOREIGN KEY (med_mnr) REFERENCES medewerkers,
                          CONSTRAINT datum_check CHECK(einddatum >= ingangsdatum)
);

insert into adressen values ('3641WJ', '46', '12-09-1998', '19-12-2021', 0297283113, 7001)
ON CONFLICT DO NOTHING;
...
alter table medewerkers DROP CONSTRAINT IF EXISTS verkoper_comm_check;
alter table medewerkers add constraint verkoper_comm_check check ((not functie = 'VERKOPER' and comm is null) or (functie = 'VERKOPER' and comm is not null));
